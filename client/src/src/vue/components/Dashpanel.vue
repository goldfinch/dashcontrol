...
<template>
  <div class="gfdashpanel__wrapper">
    <div class="gfdashpanel__primary">
      <span
        v-if="data.siteAccess != 'Anyone'"
        :title="'Site restricted access: ' + data.siteAccess"
        class="siteentry"
      ></span>
      <span
        v-if="data.env == 'dev'"
        :title="'Site in ' + data.env + ' environment type'"
        class="siteenv"
      ></span>
    </div>
    <div class="gfdashpanel__inner">
      <span
        v-if="data.page.canViewType != 'Anyone'"
        :title="'Page restricted access: ' + data.page.canViewType"
        class="pageentry"
      ></span>
      <span v-if="data.page.stagesDiffer" class="pagestate" data-type="draft"
        >Draft</span
      >
      <span
        v-else-if="!data.page.isPublished"
        class="pagestate"
        data-type="modified"
        >Modified</span
      >
      <span v-else class="pagestate" data-type="published">Published</span>
      <span class="pagetype" :title="data.page.classNamespace"
        ><span
          ><i data-type="page" :style="`opacity: .5`"
            ><img :src="`${cfg.bootstrap_icon_path}${data.page.icon}.svg`" /></i
        ></span>
        {{ data.page.className }}</span
      >
      <span
        class="pagecreated"
        :title="moment(data.page.createdAt).format('MMMM Do YYYY, HH:mm:ss')"
        ><span><i data-type="clock"></i>created</span
        >{{ moment(data.page.createdAt).fromNow() }}<i v-html="createdBy()"
      /></span>
      <span
        class="pageupdated"
        :title="moment(data.page.updatedAt).format('MMMM Do YYYY, HH:mm:ss')"
        ><span><i data-type="clock-history"></i>updated</span
        >{{ moment(data.page.updatedAt).fromNow() }}<i v-html="updatedBy()"
      /></span>
    </div>
    <ul class="gfdashpanel__sidelinks">
      ...
      <li data-i="inspect">
        <a href="javascript:void(0)"><span>Inspect</span></a>
      </li>
      <li data-i="dev">
        <a href="#" title="Dev"><span>Live</span></a>
      </li>
      ...
      <li v-if="cmsEditLink" data-i="edit">
        <a :href="cmsEditLink.content" title="Edit page"></a>
      </li>
    </ul>
  </div>
